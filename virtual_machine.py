...
for inc in file_list:
    subprocess.call(["scp", "-i", "{}".format(key_location), inc, "{}:~".format(vystup.group(1))])
subprocess.call(["virsh", "snapshot-create-as", install_name, "--disk-only", "--atomic"])
subprocess.call(["ssh", "-i", "{}".format(key_location), "root@{}:~".format(vystup.group(1)), "'python3'","'test_arrays.py'"])
