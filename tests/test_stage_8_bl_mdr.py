...
def main(disk):
    ...
    log_bl = test_utils.init_logging("blivet", 0, 8)
    log_bl_prg = test_utils.init_logging("blivet", 0, 8, "program")
    log_stage = test_utils.init_logging("stage", 0, 8)

    blivet_object = classes.BlivetInitialization(disk)

    log_stage.info("Starting test: {}".format(__file__))
    try:
        log_stage.info("Preparing disk:\t{}".format(disk))
        blivet_utils.create_disk_label(blivet_object, blivet_object.disk, "msdos")
